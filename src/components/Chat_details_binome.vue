  ...
  export default {
    ...
    setup(props) {
      ...
      const fetchMessages = async () => {
        console.log("here is id",props.id);
        const chatRef = projectFirestore.collection('messages_binome').doc(props.id);
        const messagesRef = await chatRef.data().get();
        //take the attribute list_mess 
      ...
      const sendMessage = async () => {
        if (newMessage.value.trim()) {
          ...
          await projectFirestore.collection(`messages_binome/${props.id}/list_mess`).add(message);
          newMessage.value = '';
        }
      };
